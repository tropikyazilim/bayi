...
export default function DemoListesiDataTable({
      ...
      if (tasarimParametre && tasarimParametre.deger) {
        try {
          ...
          if (tasarimVerisi.columnSizing) {
            ...
            console.log(
              "Sütun genişlikleri yüklendi:",
              tasarimVerisi.columnSizing
            );
          }

          // Filtre ayarları varsa uygula
          if (
            tasarimVerisi.columnFilters &&
            tasarimVerisi.columnFilters.length > 0
          ) {
          ...
          if (tasarimVerisi.columnVisibility) {
            ...
            console.log(
              "Sütun görünürlük bilgileri yüklendi:",
              tasarimVerisi.columnVisibility
            );
          }
          ...
          if (tasarimParametre.kayitzamani) {
            ...
            console.log(
              "Son güncelleme zamanı yüklendi:",
              tasarimParametre.kayitzamani
            );
          }
        } catch (error) {
  ...
  const updateParametersMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      if (
        response.data.updatedAyarlar &&
        response.data.updatedAyarlar.length > 0
      ) {
    ...
    if (parametersData && parametersData.length > 0) {
      ...
      if (tasarimParametre && tasarimParametre.deger) {
        try {
          ...
          if (tasarimVerisi.sorting && tasarimVerisi.sorting.length > 0) {
            console.log(
              "Sıralama bilgileri uygulanıyor:",
              tasarimVerisi.sorting
            );
            setSorting(tasarimVerisi.sorting);
          }
          ...
          if (
            tasarimVerisi.columnOrder &&
            tasarimVerisi.columnOrder.length > 0
          ) {
            console.log(
              "Sütun sıralaması uygulanıyor:",
              tasarimVerisi.columnOrder
            );
  ...
  React.useEffect(() => {
    if (table && initialColumnVisibility) {
      ...
      console.log(
        "Current table column visibility:",
        table.getState().columnVisibility
      );
      console.log(
        "Initial column visibility that should be applied:",
      ...
      Object.keys(initialColumnVisibility).forEach((columnId) => {
        ...
        if (column) {
          ...
          console.log(
            `Column ${columnId}: Should be ${
              shouldBeVisible ? "visible" : "hidden"
            }, Currently ${isCurrentlyVisible ? "visible" : "hidden"}`
          );
          ...
          if (isCurrentlyVisible !== shouldBeVisible) {
            ...
            console.log(
              `Changed visibility of ${columnId} to ${shouldBeVisible}`
            );
          }
        }
      });
          ...
          <div className="flex items-center gap-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-56">
                {" "}
                <DropdownMenuLabel>Tablo Ayarları</DropdownMenuLabel>
                <DropdownMenuSeparator />
                <DropdownMenuGroup>
                  <DropdownMenuItem onClick={handleTasarimiKaydet}>
                    Tasarımı Kaydet
                    <DropdownMenuShortcut>⇧⌘P</DropdownMenuShortcut>
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={handleVarsayilanTasarim}>
                    Tasarımı Sıfırla
